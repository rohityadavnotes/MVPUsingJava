package com.badasoftware.library.network.download.write;
    ...
    public void run() {
        ...
        try
        {
            ...
            while ((count = inputStream.read(bufferSize)) != -1)
            {
                ...
                Log.d(TAG, "PROGRESS FROM WRITE FILE : " + progress);
            ...
            /* If require send download location after download complete*/
            downloadingFileComplete(RealPathUtils.getRealPath(context, writeDownloadedFileOnThisUri));
        }
        catch (IOException iOException) {
            Log.d(TAG, "IOException 1 : " + iOException.getMessage());
        ...
        finally
        {
            if(outputStream != null)
            {
                ...
                {
                    Log.d(TAG, "IOException 2 : " + iOException.getMessage());
                    errorWhenDownloadingFile(iOException.getMessage());
                }
            }
            ...
            if(inputStream != null)
            {
                ...
                {
                    Log.d(TAG, "IOException 3 : " + iOException.getMessage());
                    errorWhenDownloadingFile(iOException.getMessage());
                }
            }
            ...
            if(parcelFileDescriptor != null)
            {
                ...
                {
                    Log.d(TAG, "IOException 4 : " + iOException.getMessage());
                    errorWhenDownloadingFile(iOException.getMessage());
                }
            }
